...
module.exports = async function (options) {
  ...
  if (need_eslint) {
    files_to_copy.push(
      {
        writeFile: async ({ dst }) => {
          let code = await eslintConfigGenerate(options.userOptions);
          log("开始生成.eslintrc.js...");
          fs.writeFile(dst, code, (err) => {
            if (err) {
              error(`.eslintrc.js内容写入文件${dst}失败,reason:${err}`);
              return;
            }
            info(`.eslintrc.js内容生成到${dst}`);
          });
        },
        to: ".eslintrc.js",
      },
  ...
  files_to_copy.push({
    writeFile: async ({ dst }) => {
      ...
      let code = await webpackConfigGenerate(options.userOptions);
